...
static int cpsw_ale_write(struct cpsw_ale *ale, int idx, u32 *ale_entry)
{
	...
	/* Caller must hold ale_lock */
	assert_spin_locked(&ale->ale_lock);
...
static int cpsw_ale_dump_mcast(struct cpsw_ale *ale, u32 *ale_entry,
		               char *buf, int len)
{
	int outlen = 0;
			       ...
			       char *buf, int len)
{
	...
	if (type == ALE_TYPE_VLAN)
		...
		outlen += cpsw_ale_get_mcast(ale_entry) ?
		  cpsw_ale_dump_mcast(ale, ale_entry, buf + outlen, len - outlen) :
		  cpsw_ale_dump_ucast(ale, ale_entry, buf + outlen, len - outlen);
				      ...
				      const char *buf, size_t count)
{
	...
	dev_info(ale->params.dev, "%s: %s called by %s with pid %d", __func__, buf,
