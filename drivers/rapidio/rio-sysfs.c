		...
		char *buf, loff_t off, size_t count)
{
	...
	if ((off & 1) && size) {
		u8 val;
		ret=rio_read_config_8(dev, off, &val);
		if(ret < 0)
			return(ret);
	...
	if ((off & 3) && size > 2) {
		u16 val;
		ret=rio_read_config_16(dev, off, &val);
		if(ret < 0)
			return(ret);
	...
	while (size > 3) {
		u32 val = (u32) ~0;
		ret=rio_read_config_32(dev, off, &val);
		if(ret < 0)
			return(ret);
		data[off - init_off + 0] = *(((u8 *) &val) + 0);
		data[off - init_off + 1] = *(((u8 *) &val) + 1);
		data[off - init_off + 2] = *(((u8 *) &val) + 2);
		data[off - init_off + 3] = *(((u8 *) &val) + 3);
		off += 4;
		size -= 4;
	}
		 ...
		 char *buf, loff_t off, size_t count)
{
	...
	if ((off & 1) && size) {
		ret=rio_write_config_8(dev, off, data[off - init_off]);
		if(ret < 0)
			return(ret);
		off++;
		size--;
	...
	if ((off & 3) && (size > 2)) {
		...
		ret=rio_write_config_16(dev, off, val);
		if(ret < 0)
			return(ret);
		off += 2;
		size -= 2;
	...
	while (size > 3) {
		...
		pval[3] |= data[off - init_off + 3];
		pval[2] |= (u32) data[off - init_off + 2];
		pval[1] |= (u32) data[off - init_off + 1];
		pval[0] |= (u32) data[off - init_off];
		ret=rio_write_config_32(dev, off, val);
		if(ret < 0)
			return(ret);
		off += 4;
		size -= 4;
...
static ssize_t config_read(struct file *fobj,
                 struct kobject *kobj,
                 struct bin_attribute *bin_attr,
                 char *buf, loff_t off, size_t count)
{
	...
	if ((off & 1) && size) {
		u8 val;
		rio_local_read_config_8(mport, off, &val);
		data[off - init_off] = val;
		off++;
		size--;
	}
	...
	if ((off & 3) && size > 2) {
		u16 val;
		rio_local_read_config_16(mport, off, &val);
		data[off - init_off] = (val >> 8) & 0xff;
		data[off - init_off + 1] = val & 0xff;
		off += 2;
		size -= 2;
	...
	while (size > 3) {
		u32 val = (u32) ~0;
		rio_local_read_config_32(mport, off, &val);
		data[off - init_off + 0] = *(((u8 *) &val) + 0);
		data[off - init_off + 1] = *(((u8 *) &val) + 1);
		data[off - init_off + 2] = *(((u8 *) &val) + 2);
		data[off - init_off + 3] = *(((u8 *) &val) + 3);
...
static ssize_t config_write(struct file *fobj,
                  struct kobject *kobj,
                  struct bin_attribute *bin_attr,
                  char *buf, loff_t off, size_t count)
{
	...
	if ((off & 1) && size) {
		rio_local_write_config_8(mport, off, data[off - init_off]);
		off++;
		size--;
	}
	...
	if ((off & 3) && (size > 2)) {
		u16 val = data[off - init_off + 1];
		val |= (u16) data[off - init_off] << 8;
		rio_local_write_config_16(mport, off, val);
		off += 2;
		size -= 2;
	}
	...
	while (size > 3) {
		...
		pval[1] |= (u32) data[off - init_off + 1];
		pval[0] |= (u32) data[off - init_off];
		rio_local_write_config_32(mport, off, val);
		off += 4;
		size -= 4;
	}
...
static BIN_ATTR_RW(config, 0x1000000);
...
static struct bin_attribute *rio_mport_binattrs[] = {
	&bin_attr_config,
	NULL,
};
