...
{
	struct sa_intf *sa_intf = intf_priv;
	struct sa_device *sa_dev = sa_intf->sa_device;
	printk("%s() called for interface %s\n", __func__, sa_intf->net_device->name);

	netif_napi_del(&sa_intf->tx_pipe.dma_poll_napi);

	devm_kfree(sa_dev->dev, sa_intf);
	return 0;
}
