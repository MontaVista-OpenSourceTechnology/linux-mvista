...
static void emac_dump_regs(struct emac_priv *priv)
{
	...
	dev_info(emac_dev, "EMAC: TXID: %08X %s, RXID: %08X %s\n", emac_read(EMAC_TXIDVER), ((emac_read(EMAC_TXCONTROL)) ? "enabled" : "disabled"), emac_read(EMAC_RXIDVER), ((emac_read(EMAC_RXCONTROL)) ? "enabled" : "disabled"));
	dev_info(emac_dev, "EMAC: TXIntRaw:%08X, TxIntMasked: %08X, TxIntMasSet: %08X\n", emac_read(EMAC_TXINTSTATRAW), emac_read(EMAC_TXINTSTATMASKED), emac_read(EMAC_TXINTMASKSET));
	dev_info(emac_dev, "EMAC: RXIntRaw:%08X, RxIntMasked: %08X, RxIntMasSet: %08X\n", emac_read(EMAC_RXINTSTATRAW), emac_read(EMAC_RXINTSTATMASKED), emac_read(EMAC_RXINTMASKSET));
	dev_info(emac_dev, "EMAC: MacIntRaw:%08X, MacIntMasked: %08X,  MacInVector=%08X\n", emac_read(EMAC_MACINTSTATRAW), emac_read(EMAC_MACINTSTATMASKED), emac_read(EMAC_MACINVECTOR));
	dev_info(emac_dev, "EMAC: EmuControl:%08X, FifoControl: %08X\n", emac_read(EMAC_EMCONTROL), emac_read(EMAC_FIFOCONTROL));
	dev_info(emac_dev, "EMAC: MBPEnable:%08X, RXUnicastSet: %08X, RXMaxLen=%08X\n", emac_read(EMAC_RXMBPENABLE), emac_read(EMAC_RXUNICASTSET), emac_read(EMAC_RXMAXLEN));
	dev_info(emac_dev, "EMAC: MacControl:%08X, MacStatus: %08X, MacConfig=%08X\n", emac_read(EMAC_MACCONTROL), emac_read(EMAC_MACSTATUS), emac_read(EMAC_MACCONFIG));
...
static int emac_set_coalesce(struct net_device *ndev, struct ethtool_coalesce *coal)
{
	...
	case EMAC_VERSION_2:
		...
		if (coal_intvl > EMAC_DM646X_CMINTMAX_INTVL) {
			...
			if (addnl_dvdr > 1) {
				prescale *= addnl_dvdr;
				if (coal_intvl > (EMAC_DM646X_CMINTMAX_INTVL * addnl_dvdr))
					coal_intvl = (EMAC_DM646X_CMINTMAX_INTVL * addnl_dvdr);
			} else {
				addnl_dvdr = 1;
...
static void emac_update_phystatus(struct emac_priv *priv)
{
	...
	cur_duplex = (mac_control & EMAC_MACCONTROL_FULLDUPLEXEN) ? DUPLEX_FULL : DUPLEX_HALF;
	if (priv->phy_mask)
	...
	if (priv->speed == SPEED_1000 && (priv->version == EMAC_VERSION_2)) {
		mac_control = emac_read(EMAC_MACCONTROL);
		mac_control |= (EMAC_DM646X_MACCONTORL_GIG | EMAC_DM646X_MACCONTORL_GIGFORCE);
	} else {
		/* Clear the GIG bit and GIGFORCE bit */
		mac_control &= ~(EMAC_DM646X_MACCONTORL_GIGFORCE | EMAC_DM646X_MACCONTORL_GIG);
...
/**
 * cpgmac_hash_add: Hash function to add mac addr from hash table
 * @priv: The CPGMAC private adapter structure
 ...
 *
 */
static int cpgmac_hash_add(struct emac_priv *priv, u8 *mac_addr)
{
	struct device *emac_dev = &priv->ndev->dev;
	u32 rc = 0;
	u32 hash_bit;
	...
	if (hash_value >= EMAC_NUM_MULTICAST_BITS) {
		if (netif_msg_drv(priv))
		    dev_err(emac_dev, "CPGMAC: cpgmac_hash_add(): Invalid Hash %08x, should not be greater than %08x", hash_value, (EMAC_NUM_MULTICAST_BITS - 1));
		return -1;
	}
...
/**
 * cpgmac_hash_del: Hash function to delete mac addr from hash table
 * @priv: The CPGMAC private adapter structure
 * mac_addr: mac address to delete from hash table
 *
 ...
 *
 */
static int cpgmac_hash_del(struct emac_priv *priv, u8 *mac_addr)
{
	u32 hash_value;
	u32 hash_bit;
...
static void emac_add_mcast(struct emac_priv *priv, u32 action, u8 *mac_addr)
{
	...
	case EMAC_MULTICAST_ADD:
		update = cpgmac_hash_add(priv, mac_addr);
		break;
	case EMAC_MULTICAST_DEL:
		update = cpgmac_hash_del(priv, mac_addr);
		break;
	...
	case EMAC_ALL_MULTI_CLR:
		...
		priv->mac_hash1 = 0;
		priv->mac_hash2 = 0;
		memset(&(priv->multicast_hash_cnt[0]), 0, sizeof(priv->multicast_hash_cnt[0]) * EMAC_NUM_MULTICAST_BITS);
		break;
	default:
		if (netif_msg_drv(priv))
			dev_err(emac_dev, "CPGMAC: add_mcast: bad operation %d", action);
		break;
	}
...
static void emac_dev_mcast_set(struct net_device *ndev)
{
	...
	if (ndev->flags & IFF_PROMISC) {
		...
		if ((ndev->flags & IFF_ALLMULTI) || (netdev_hw_addr_list_count(mcptr) > EMAC_DEF_MAX_MULTICAST_ADDRESSES)) {
		...
		if (netdev_hw_addr_list_count(mcptr) > 0) {
			...
			netdev_for_each_mc_addr(mc_ptr, ndev) {
				...
				emac_add_mcast(priv, EMAC_MULTICAST_ADD, (u8 *)dmi_addr);
			}
		} else {
			mbp_enable = (mbp_enable & ~EMAC_MBP_RXMCAST);
			emac_add_mcast(priv, EMAC_ALL_MULTI_CLR, NULL);
...
static void emac_int_enable(struct emac_priv *priv)
{
	...
	if (priv->version == EMAC_VERSION_2) {
		...
		emac_write(EMAC_DM646X_MACEOIVECTOR, EMAC_DM646X_MAC_EOI_C0_RXEN);

		/* ack txen- only then a new pulse will be generated */
		emac_write(EMAC_DM646X_MACEOIVECTOR, EMAC_DM646X_MAC_EOI_C0_TXEN);
...
#define EMAC_RX_BD_MEM(priv)    ((priv)->emac_ctrl_ram + (((priv)->ctrl_ram_size) >> 1))
...
static int emac_init_txch(struct emac_priv *priv, u32 ch)
{
	...
	/* allocate memory for TX CPPI channel on a 4 byte boundary */
	txch->tx_complete = kzalloc(txch->service_max * sizeof(u32), GFP_KERNEL);
	 ...
	 * boundary for future requirements
	 */
	...
	mem = (void __force __iomem *) (((u32 __force)txch->bd_mem + BD_ALIGNMENT) & ~BD_ALIGNMENT);
 ...
 *
 */
static int emac_net_tx_complete(struct emac_priv *priv, void **net_data_tokens, int num_tokens, u32 ch)
{
	u32 cnt;
...
static void emac_txch_teardown(struct emac_priv *priv, u32 ch)
{
	...
	while ((emac_read(EMAC_TXCP(ch)) & EMAC_TEARDOWN_VALUE) != EMAC_TEARDOWN_VALUE) {
	...
	if (1 == txch->queue_active) {
		...
		while (curr_bd != NULL) {
			emac_net_tx_complete(priv, (void __force *) &curr_bd->buf_token, 1, ch);
...
static int emac_tx_bdproc(struct emac_priv *priv, u32 ch, u32 budget)
{
	...
	if (unlikely(1 == txch->teardown_pending)) {
		if (netif_msg_tx_err(priv) && net_ratelimit())
			dev_err(emac_dev, "CPGMAC:emac_tx_bdproc: teardown pending\n");
	...
	curr_bd = txch->active_queue_head;
	if (NULL == curr_bd) {
		emac_write(EMAC_TXCP(ch), emac_virt_to_phys(txch->last_hw_bdprocessed));
		txch->no_active_pkts++;
	...
	while ((curr_bd) && ((frame_status & EMAC_CPPI_OWNERSHIP_BIT) == 0) && (pkts_processed < budget)) {
	...
	emac_net_tx_complete(priv, (void *)&txch->tx_complete[0], tx_complete_cnt, ch);
...
static int emac_send(struct emac_priv *priv, struct emac_netpktobj *pkt, u32 ch)
{
	...
	curr_bd->h_next = 0;
	curr_bd->next = NULL;
	curr_bd->mode = (EMAC_CPPI_SOP_BIT | EMAC_CPPI_OWNERSHIP_BIT | EMAC_CPPI_EOP_BIT | pkt->pkt_length);
...
static int emac_dev_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	if (unlikely(ret_code != 0)) {
		if (ret_code == EMAC_ERR_TX_OUT_OF_BD) {
			if (netif_msg_tx_err(priv) && net_ratelimit())
				dev_err(emac_dev, "CPGMAC: xmit() fatal err. Out of TX BD's");
 ...
 */
static void *emac_net_alloc_rx_buf(struct emac_priv *priv, int buf_size, void **data_token, u32 ch)
{
	struct net_device *ndev = priv->ndev;
	struct device *emac_dev = &ndev->dev;
...
static int emac_init_rxch(struct emac_priv *priv, u32 ch, char *param)
{
	...
	mem = (void __force __iomem *) (((u32 __force)rxch->bd_mem + BD_ALIGNMENT) & ~BD_ALIGNMENT);
	...
	for (cnt = 0; cnt < rxch->num_bd; cnt++) {
		...
		curr_bd->data_ptr = emac_net_alloc_rx_buf(priv, rxch->buf_size, (void __force **)&curr_bd->buf_token, EMAC_DEF_RX_CH);
		if (curr_bd->data_ptr == NULL) {
			dev_err(emac_dev, "CPGMAC: RX buf mem alloc failed for ch %d\n", ch);
			kfree(rxch);
			return -ENOMEM;
		}
		...
		BD_CACHE_WRITEBACK_INVALIDATE((u32)curr_bd, EMAC_BD_LENGTH_FOR_CACHE);
...
static void emac_rxch_teardown(struct emac_priv *priv, u32 ch)
{
	...
	while ((emac_read(EMAC_RXCP(ch)) & EMAC_TEARDOWN_VALUE) != EMAC_TEARDOWN_VALUE) {
		/* wait till tx teardown complete */
		cpu_relax(); /* TODO: check if this helps ... */
		--teardown_cnt;
		if (0 == teardown_cnt) {
	...
	if (rxch) {
		...
		while (curr_bd) {
			if (curr_bd->buf_token)
				dev_kfree_skb_any((struct sk_buff *)curr_bd->buf_token);
			curr_bd = curr_bd->next;
		}
		if (rxch->bd_mem)
			rxch->bd_mem = NULL;
 ...
 *
 */
static void emac_addbd_to_rx_queue(struct emac_priv *priv, u32 ch, struct emac_rx_bd __iomem *curr_bd, char *buffer, void *buf_token)
{
	struct emac_rxch *rxch = priv->rxch[ch];
 ...
 */
static int emac_net_rx_cb(struct emac_priv *priv, struct emac_netpktobj *net_pkt_list)
{
	struct sk_buff *p_skb;
...
static int emac_rx_bdproc(struct emac_priv *priv, u32 ch, u32 budget)
{
	...
	while ((curr_bd) && ((frame_status & EMAC_CPPI_OWNERSHIP_BIT) == 0) && (pkts_processed < budget)) {
		...
		curr_pkt->num_bufs = 1;
		curr_pkt->pkt_length = (frame_status & EMAC_RX_BD_PKT_LENGTH_MASK);
		...
		/* recycle BD */
		emac_addbd_to_rx_queue(priv, ch, last_bd, new_buffer, new_buf_token);
...
static int emac_hw_enable(struct emac_priv *priv)
{
	...
	for (ch = 0; ch < EMAC_DEF_MAX_RX_CH; ch++) {
		...
		rxch->queue_active = 1;
		emac_write(EMAC_RXHDP(ch), emac_virt_to_phys(rxch->active_queue_head));
	}
...
static int emac_dev_open(struct net_device *ndev)
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (res != NULL) {
			if (request_irq(res->start, emac_irq, IRQD_IRQ_DISABLED, ndev->name, ndev))
				goto rollback;
		}
	}
	...
	if (priv->phy_mask) {
		...
		priv->phydev = phy_connect(ndev, dev_name(&priv->phydev->mdio.dev), &emac_adjust_link, PHY_INTERFACE_MODE_MII);
		...
		printk(KERN_INFO "%s: attached PHY driver [%s] (mii_bus:phy_addr=%s, id=%x)\n", ndev->name, priv->phydev->drv->name, dev_name(&priv->phydev->mdio.dev), priv->phydev->phy_id);
	} else {
...
static struct net_device_stats *emac_dev_getnetstats(struct net_device *ndev)
{
	...
	priv->net_dev_stats.rx_length_errors += (emac_read(EMAC_RXOVERSIZED) + emac_read(EMAC_RXJABBER) + emac_read(EMAC_RXUNDERSIZED));
	emac_write(EMAC_RXOVERSIZED, stats_clear_mask);
	emac_write(EMAC_RXJABBER, stats_clear_mask);
	emac_write(EMAC_RXUNDERSIZED, stats_clear_mask);

	priv->net_dev_stats.rx_over_errors += (emac_read(EMAC_RXSOFOVERRUNS) + emac_read(EMAC_RXMOFOVERRUNS));
 ...
 */
static int CPGMAC_emac_probe(struct platform_device *pdev)
{
	int i, rc = 0;
	struct resource *res;
	struct net_device *ndev;
	...
	if (!request_mem_region(res->start, size, ndev->name)) {
		dev_err(emac_dev, "CPGMAC: failed request_mem_region() for regs\n");
		rc = -ENXIO;
		goto probe_quit;
	}
...
#else
	if (of_property_read_u32(np, "puma1,ctrl_mod_reg_offset", &ctrl_mod_reg_offset))
	...
	/* MII/Phy initialisation, mdio bus registration */
	...
	emac_mii->irq[0]   = PHY_POLL,
	emac_mii->irq[1]   = PHY_POLL, //mii_irqs
