				       ...
				       struct ethtool_link_ksettings *cmd)
{
	...
	slave = (struct cpsw_slave *)phy->priv;
	if (!slave)
		return -EINVAL;
...
static void cpsw_adjust_link(struct net_device *n_dev)
{
	...
	slave = (struct cpsw_slave *) n_dev->phydev->priv;
	if (!slave)
		return;

	netcp = netdev_priv(n_dev);
...
static void cpsw_timer(struct timer_list *t)
{
	struct cpsw_intf *cpsw_intf = from_timer(cpsw_intf, t, timer);
	struct cpsw_priv *cpsw_dev = cpsw_intf->cpsw_priv;
...
static int cpsw_open(void *intf_priv, struct net_device *ndev)
{
	...
	timer_setup(&cpsw_intf->timer, cpsw_timer, 0);
